...
const FollowButton = ({ vendorId, initialFollowersCount = 0, className = '' }) => {
  ...
  const checkFollowStatus = async () => {
    try {
      ...
      const vendorIdStr = typeof vendorId === 'object' ? (vendorId._id || vendorId) : vendorId;
      ...
      const response = await fetch(`/api/follows/is-following/${vendorIdStr}`, {
        headers: {
    ...
    try {
      if (isFollowing) {
        ...
        const response = await fetch('/api/follows/unfollow', {
          ...
          body: JSON.stringify({ vendorId: vendorIdStr })
        });
