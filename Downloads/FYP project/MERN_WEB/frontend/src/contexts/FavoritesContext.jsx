...
export const FavoritesProvider = ({ children }) => {
  ...
  const fetchFavorites = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filter out favorites where the product has been deleted (null)
        const validFavorites = (data.favorites || []).filter(fav => fav.product);
        console.log('FavoritesContext: Fetched favorites:', validFavorites.length, 'valid items');
  ...
  const toggleFavorite = async (productId) => {
    const isFavorited = favorites.some(fav => fav.product && fav.product._id === productId);
  ...
  const isInFavorites = (productId) => {
    return favorites.some(fav => fav.product && fav.product._id === productId);
  };
