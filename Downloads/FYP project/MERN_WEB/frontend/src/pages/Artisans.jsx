    ...
    const fetchVendors = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Fetch vendor review stats for each vendor
          const vendorsWithReviews = await Promise.all(
            vendorData.map(async (vendor) => {
              try {
                ...
                console.error(`Error fetching reviews for vendor ${vendor.user._id}:`, error)
                return {
                  ...vendor,
                  vendorReviewStats: { averageOverallRating: 0, totalReviews: 0 }
                }
              }
            })
          )
          ...
          const transformedVendors = vendorsWithReviews.map(vendor => {
            ...
            location: formatLocation(vendor.location),
              ...
              vendorReviewStats: vendor.vendorReviewStats, // Add this for debugging
