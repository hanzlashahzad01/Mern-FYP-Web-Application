...
router.post('/visual-search', uploadSingle('image'), convertUploadedFiles, async (req, res) => {
  try {
    ...
    // Step 2: Search for similar products in the database
    const { description, keywords, suggestedCategory } = analysis;

    // Build search query
    // We'll search in name, description, category and tags
    const searchTerms = [...keywords, description].join(' ');

    const filter = {
      isActive: true,
      $or: [
        { name: { $regex: keywords.join('|'), $options: 'i' } },
        { description: { $regex: keywords[0], $options: 'i' } }, // Use first few keywords for description search
        { category: suggestedCategory },
        { tags: { $in: keywords.map(tag => new RegExp(tag, 'i')) } }
      ]
    };
